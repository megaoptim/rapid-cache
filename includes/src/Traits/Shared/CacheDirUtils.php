...
trait CacheDirUtils
{
    ...
    public function cacheDir($rel_path = '')
    {
        ...
        if ($this->isAdvancedCache()) {
            $cache_dir = defined('RAPID_CACHE_DIR') ? RAPID_CACHE_DIR : '';
        } elseif (!empty($this->cache_sub_dir)) {
            $cache_dir = $this->wpContentBaseDirTo($this->cache_sub_dir);
        }
        if (empty($cache_dir)) {
            throw new \Exception(__('Unable to determine cache directory location.', 'rapid-cache'));
        }
     ...
     *    i.e., those that match a specific regex pattern.
     *
     * @since 1.0.0 Updated 151002 w/ multisite compat. improvements.
     *
    ...
    public function deleteFilesFromCacheDir($regex, $check_max_age = false)
    {
        ...
        if ($check_max_age && $this->isAdvancedCache()) {
            throw new \Exception(__('Invalid argument; isAdvancedCache!', 'rapid-cache'));
        }
        ...
        # if(WP_DEBUG) file_put_contents(WP_CONTENT_DIR.'/'.mb_strtolower(MEGAOPTIM_RAPID_CACHE_SHORT_NAME).'-debug.log', print_r($regex, TRUE)."\n".print_r($cache_dir_tmp_regex, TRUE)."\n\n", FILE_APPEND);
        ...
        if (!rename($cache_dir, $cache_dir_tmp)) {
            throw new \Exception(sprintf(__('Unable to delete files. Rename failure on directory: `%1$s`.', 'rapid-cache'), $cache_dir));
        }
        foreach (($_dir_regex_iteration = $this->dirRegexIteration($cache_dir_tmp, $cache_dir_tmp_regex)) as $_resource) {
            ...
            if ($_resource_type !== 'dir' && mb_strpos($_sub_path_name, '/') === false) {
                continue; // Don't delete links/files in the immediate directory; e.g. `[MEGAOPTIM_RAPID_CACHE_SHORT_NAME]-advanced-cache` or `.htaccess`, etc.
            ...
            switch ($_resource_type) {// Based on type; i.e., `link`, `file`, `dir`.
                case 'link': // Symbolic links; i.e., 404 errors.
                    ...
                    if (!unlink($_path_name)) {
                        ...
                        throw new \Exception(sprintf(__('Unable to delete symlink: `%1$s`.', 'rapid-cache'), $_path_name));
                    }
                ...
                case 'file': // Regular files; i.e., not symlinks.
                    ...
                    if (!unlink($_path_name)) {
                        $this->tryErasingAllFilesDirsIn($cache_dir_tmp, true); // Cleanup if possible.
                        throw new \Exception(sprintf(__('Unable to delete file: `%1$s`.', 'rapid-cache'), $_path_name));
                    }
                ...
                case 'dir': // A regular directory; i.e., not a symlink.
                    ...
                    if (!rmdir($_path_name)) {
                        $this->tryErasingAllFilesDirsIn($cache_dir_tmp, true); // Cleanup if possible.
                        throw new \Exception(sprintf(__('Unable to delete dir: `%1$s`.', 'rapid-cache'), $_path_name));
                    }
                ...
                default: // Something else that is totally unexpected here.
                    $this->tryErasingAllFilesDirsIn($cache_dir_tmp, true); // Cleanup if possible.
                    throw new \Exception(sprintf(__('Unexpected resource type: `%1$s`.', 'rapid-cache'), $_resource_type));
            }
        }
        ...
        if (!rename($cache_dir_tmp, $cache_dir)) {
            $this->tryErasingAllFilesDirsIn($cache_dir_tmp, true); // Cleanup if possible.
            throw new \Exception(sprintf(__('Unable to delete files. Rename failure on tmp directory: `%1$s`.', 'rapid-cache'), $cache_dir_tmp));
        }
        /* ------- End lock state... ------------- */
    ...
    public function deleteFilesFromHostCacheDir(
        ...
        // See: <https://github.com/websharks/rapid-cache/issues/608>
        ...
        if (!$host_token) { // Must have a host in the sub-routine below.
            throw new \Exception(__('Invalid argument; host token empty!', 'rapid-cache'));
        }
        if ($check_max_age && $this->isAdvancedCache()) {
            throw new \Exception(__('Invalid argument; isAdvancedCache!', 'rapid-cache'));
        }
        ...
        foreach (['http', 'https'] as $_host_scheme) {
            ...
            if (!rename($_host_cache_dir, $_host_cache_dir_tmp)) {
                throw new \Exception(sprintf(__('Unable to delete files. Rename failure on tmp directory: `%1$s`.', 'rapid-cache'), $_host_cache_dir));
            }
            ...
            if (!rename($_host_cache_dir_tmp, $_host_cache_dir)) {
                $this->tryErasingAllFilesDirsIn($_host_cache_dir_tmp, true); // Cleanup if possible.
                throw new \Exception(sprintf(__('Unable to delete files. Rename failure on tmp directory: `%1$s`.', 'rapid-cache'), $_host_cache_dir_tmp));
            }
        }
     ...
     *    including `[MEGAOPTIM_RAPID_CACHE_SHORT_NAME]-` prefixed files; or anything else for that matter.
     *
     * @since 1.0.0 Updated 151002 w/ multisite compat. improvements.
     *
    ...
    public function deleteAllFilesDirsIn($dir, $delete_dir_too = false)
    {
        ...
        if (!rename($dir, $dir_temp)) {
            throw new \Exception(sprintf(__('Unable to delete all files/dirs. Rename failure on tmp directory: `%1$s`.', 'rapid-cache'), $dir));
        }
        ...
        if (!rename($dir_temp, $dir)) {
            $this->tryErasingAllFilesDirsIn($dir_temp, true); // Cleanup if possible.
            throw new \Exception(sprintf(__('Unable to delete all files/dirs. Rename failure on tmp directory: `%1$s`.', 'rapid-cache'), $dir_temp));
        }
        if ($delete_dir_too) {
            if (!rmdir($dir)) {
                throw new \Exception(sprintf(__('Unable to delete directory: `%1$s`.', 'rapid-cache'), $dir));
            }
    ...
    public function eraseAllFilesDirsIn($dir, $erase_dir_too = false)
    {
        ...
        foreach (($_dir_regex_iteration = $this->dirRegexIteration($dir, '/.+/u')) as $_resource) {
            ...
            switch ($_resource_type) {// Based on type; i.e., `link`, `file`, `dir`.
                case 'link': // Symbolic links; i.e., 404 errors.
                    ...
                    if (!unlink($_path_name)) {
                        throw new \Exception(sprintf(__('Unable to erase symlink: `%1$s`.', 'rapid-cache'), $_path_name));
                    }
                ...
                case 'file': // Regular files; i.e., not symlinks.
                    ...
                    if (!unlink($_path_name)) {
                        throw new \Exception(sprintf(__('Unable to erase file: `%1$s`.', 'rapid-cache'), $_path_name));
                    }
                ...
                case 'dir': // A regular directory; i.e., not a symlink.
                    ...
                    if (!rmdir($_path_name)) {
                        throw new \Exception(sprintf(__('Unable to erase dir: `%1$s`.', 'rapid-cache'), $_path_name));
                    }
        ...
        if ($erase_dir_too) {
            if (!rmdir($dir)) {
                throw new \Exception(sprintf(__('Unable to erase directory: `%1$s`.', 'rapid-cache'), $dir));
            }
