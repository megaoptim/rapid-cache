...
class AutoCache extends AbsBase {
	...
	protected function run() {
		...
		foreach ( $blogs as $_blog ) {
			$_blog_sitemap_urls = $_wp_default_sitemap_urls = $_blog_other_urls = $_blog_urls = [];
			...
			if ( function_exists( '\wp_sitemaps_get_server' ) ) {
				$_sitemaps = \wp_sitemaps_get_server()->index->get_sitemap_list();
				foreach ( $_sitemaps as $_sitemap ) {
					$_sitemap_urls = $this->getSitemapUrlsDeep( $_sitemap['loc'] );
					if ( ! empty( $_sitemap_urls ) ) {
						$_wp_default_sitemap_urls = array_merge( $_wp_default_sitemap_urls, $_sitemap_urls );
					}
				}
			}
			...
			wp_remote_get(
				...
				[
					...
					'user-agent' => $this->plugin->options['auto_cache_user_agent'] .
					                '; ' . MEGAOPTIM_RAPID_CACHE_GLOBAL_NS . ' ' . MEGAOPTIM_RAPID_CACHE_VERSION,
				]
			)
		);
	}
	...
	protected function logAutoCacheUrl( $url, $wp_remote_get_response ) {
		...
		$auto_cache_log_file = $cache_dir . '/' . mb_strtolower( MEGAOPTIM_RAPID_CACHE_SHORT_NAME ) . '-auto-cache.log';

		if ( is_file( $auto_cache_log_file ) && ! is_writable( $auto_cache_log_file ) ) {
			throw new \Exception( sprintf( __( 'Auto-cache log file is NOT writable: `%1$s`. Please set permissions to `644` (or higher). `666` might be needed in some cases.', 'rapid-cache' ), $auto_cache_log_file ) );
		}
	...
	protected function getSitemapUrlsDeep( $sitemap, $___recursive = false ) {
		...
		if ( ! $allow_url_fopen || $this->plugin->applyWpFilters( MEGAOPTIM_RAPID_CACHE_GLOBAL_NS . '_auto_cache_sitemap_force_wp_http_api', false ) ) { // Try the WP HTTP API instead.
		...
		if ( ! $allow_url_fopen || $this->plugin->applyWpFilters( MEGAOPTIM_RAPID_CACHE_GLOBAL_NS . '_auto_cache_sitemap_force_wp_http_api', false ) ) {
