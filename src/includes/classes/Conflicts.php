    ...
    protected static function doCheck()
    {
        ...
        foreach ($active_plugins as $_active_plugin_basename) {
            ...
            if (in_array($_active_plugin_slug, $conflicting_plugin_slugs, true)) {
                if (empty($GLOBALS[GLOBAL_NS.'_uninstalling']) && is_admin() && in_array($_active_plugin_slug, ['rapid-cache', 'rapid-cache-pro', 'zencache', 'zencache-pro', 'quick-cache', 'quick-cache-pro'], true)) {
    ...
    protected function maybeEnqueueNotice()
    {
        ...
        add_action('all_admin_notices', function () {
            ...
            if (strcasecmp($this_plugin_name, $conflicting_plugin_name) === 0) {
                $this_plugin_name = $this_plugin_name.' '.__('Pro', 'rapid-cache');
                $conflicting_plugin_name = $conflicting_plugin_name.' '.__('Lite', 'rapid-cache');
            ...
            echo '<div class="error">'.// Error notice.
                 ...
                 '      '.sprintf(__('<strong>%1$s</strong> is NOT running. A conflicting plugin, <strong>%2$s</strong>, is currently active. Please deactivate the %2$s plugin to clear this message.', 'rapid-cache'), esc_html($this_plugin_name), esc_html($conflicting_plugin_name)).
                 '   </p>'.
                 '</div>';
        });
    }
