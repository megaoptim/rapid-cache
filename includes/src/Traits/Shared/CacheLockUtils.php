...
trait CacheLockUtils
{
    ...
    {
        if ($this->applyWpFilters(MEGAOPTIM_RAPID_CACHE_GLOBAL_NS.'\\share::disable_cache_locking', false)
            || $this->applyWpFilters(MEGAOPTIM_RAPID_CACHE_GLOBAL_NS.'_disable_cache_locking', false)
        ) {
            return false; // Disabled cache locking.
        }
        if (!($wp_config_file = $this->findWpConfigFile())) {
            throw new \Exception(__('Unable to find the wp-config.php file.', 'rapid-cache'));
        }
        $lock_type = 'flock'; // Default lock type.
        $lock_type = $this->applyWpFilters(MEGAOPTIM_RAPID_CACHE_GLOBAL_NS.'\\share::cache_lock_lock_type', $lock_type);
        $lock_type = $this->applyWpFilters(MEGAOPTIM_RAPID_CACHE_GLOBAL_NS.'_cache_lock_type', $lock_type);
        ...
        $mutex     = $tmp_dir.'/'.MEGAOPTIM_RAPID_CACHE_SLUG.'-'.$inode_key.'.lock';

        if (!($resource = fopen($mutex, 'wb')) || !flock($resource, LOCK_EX)) {
            throw new \Exception(__('Unable to obtain an exclusive lock.', 'rapid-cache'));
        }
     ...
     *
     * @since 1.0.0 Updated 151002 to remove the `array` typecast.
     *
