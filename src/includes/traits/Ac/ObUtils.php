...
trait ObUtils
{
    ...
    public function maybeStartOutputBuffering()
    {
        ...
        if ((!IS_PRO || !RAPID_CACHE_WHEN_LOGGED_IN) && $this->isLikeUserLoggedIn()) {
        ...
        if (RAPID_CACHE_EXCLUDE_AGENTS && !empty($_SERVER['HTTP_USER_AGENT']) && (!IS_PRO || !$this->isAutoCacheEngine())) {
        ...
        if (IS_PRO && RAPID_CACHE_WHEN_LOGGED_IN === 'postload' && $this->isLikeUserLoggedIn()) {
    ...
    public function outputBufferCallbackHandler($buffer, $phase)
    {
        ...
        if ((!IS_PRO || !RAPID_CACHE_WHEN_LOGGED_IN) && $this->is_user_logged_in) {
        ...
        if (empty($cache_file_dir_writable) && !is_writable($cache_file_dir)) {
            throw new \Exception(sprintf(__('Cache directory not writable. %1$s needs this directory please: `%2$s`. Set permissions to `755` or higher; `777` might be needed in some cases.', 'rapid-cache'), NAME, $cache_file_dir));
        }
        ...
        if (RAPID_CACHE_DEBUGGING_ENABLE && $this->isHtmlXmlDoc($cache)) {
            ...
            $DebugNotes = new Classes\Notes();
            $DebugNotes->addAsciiArt(sprintf(__('%1$s is Fully Functional', 'rapid-cache'), NAME));
            $DebugNotes->addLineBreak();

            if (IS_PRO && RAPID_CACHE_WHEN_LOGGED_IN && $this->user_token) {
                $DebugNotes->add(__('Cache File User Token', 'rapid-cache'), $this->user_token);
            }
            if (IS_PRO && RAPID_CACHE_MOBILE_ADAPTIVE_SALT_ENABLE && RAPID_CACHE_MOBILE_ADAPTIVE_SALT && $this->mobile_adaptive_salt) {
            ...
            $DebugNotes->add(__('Cache File Generated Via', 'rapid-cache'), IS_PRO && $this->isAutoCacheEngine() ? __('Auto-Cache Engine', 'rapid-cache') : __('HTTP request', 'rapid-cache'));
            ...
            if (IS_PRO && RAPID_CACHE_WHEN_LOGGED_IN && $this->cache_max_age < $this->nonce_cache_max_age && preg_match('/\b(?:_wpnonce|akismet_comment_nonce)\b/u', $cache)) {
        ...
        throw new \Exception(sprintf(__('%1$s: failed to write cache file for: `%2$s`; possible permissions issue (or race condition), please check your cache directory: `%3$s`.', 'rapid-cache'), NAME, $_SERVER['REQUEST_URI'], RAPID_CACHE_DIR));
    }
}
