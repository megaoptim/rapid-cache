...
trait HookUtils
{
    ...
    public function hookId($function)
    {
        ...
        throw new \Exception(__('Invalid hook.', 'rapid-cache'));
    }
    ...
    public function addHook($hook, $function, $priority = 10, $accepted_args = 1)
    {
        ...
        if (mb_stripos($hook, 'zencache') === 0) {
            $hook = MEGAOPTIM_RAPID_CACHE_GLOBAL_NS.mb_substr($hook, mb_strlen('zencache'));
        }
    ...
    */
    ...
    public function doWpAction($hook)
    {
        ...
        if (mb_stripos($hook, MEGAOPTIM_RAPID_CACHE_GLOBAL_NS) === 0) {
            $zencache_filter  = 'zencache'.mb_substr($hook, mb_strlen(MEGAOPTIM_RAPID_CACHE_GLOBAL_NS));
    ...
    public function applyWpFilters($hook)
    {
        ...
        if (mb_stripos($hook, MEGAOPTIM_RAPID_CACHE_GLOBAL_NS) === 0) {
            $zencache_hook    = 'zencache'.mb_substr($hook, mb_strlen(MEGAOPTIM_RAPID_CACHE_GLOBAL_NS));
            $zencache_args    = $args; // Use a copy of the args.
