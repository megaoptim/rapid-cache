...
trait InstallUtils
{
    ...
    public function activate()
    {
        ...
        if (MEGAOPTIM_RAPID_CACHE_IS_PRO && (!$this->options['pro_update_username'] || !$this->options['pro_update_password'])) {
            $configure_pro_updater_url = add_query_arg(urlencode_deep(['page' => MEGAOPTIM_RAPID_CACHE_GLOBAL_NS, MEGAOPTIM_RAPID_CACHE_GLOBAL_NS.'_configure_pro_updater' => 1]), network_admin_url('/admin.php')).'#'.MEGAOPTIM_RAPID_CACHE_SLUG.'-configure-pro-updater';
        ...
        if (!$this->options['welcomed'] && !$this->options['enable']) {
            $settings_url = add_query_arg(urlencode_deep(['page' => MEGAOPTIM_RAPID_CACHE_GLOBAL_NS]), network_admin_url('/admin.php'));
            $this->enqueueMainNotice(sprintf(__('<strong>%1$s</strong> successfully installed! :-) <strong>Please <a href="%2$s">enable caching and review options</a>.</strong>', 'rapid-cache'), esc_html(MEGAOPTIM_RAPID_CACHE_NAME), esc_attr($settings_url)), ['push_to_top' => true]);
    ...
    public function checkVersion()
    {
        ...
        if (version_compare($prev_version, MEGAOPTIM_RAPID_CACHE_VERSION, '>=')) {
        ...
        $this->updateOptions(['version' => MEGAOPTIM_RAPID_CACHE_VERSION], false);
        // Retain all options in database for VS Upgrade routine.

        new Classes\VsUpgrades($prev_version);

        $this->updateOptions(['version' => MEGAOPTIM_RAPID_CACHE_VERSION], true);
        ...
        if (is_admin()) { // Only if in an admin area.
            $this->enqueueMainNotice(sprintf(__('<strong>%1$s:</strong> detected a new version of itself. Recompiling w/ latest version... wiping the cache... all done :-)', 'rapid-cache'), esc_html(MEGAOPTIM_RAPID_CACHE_NAME)), ['push_to_top' => true]);
        }
    ...
    public function uninstall()
    {
        ...
        if (!defined('WP_UNINSTALL_PLUGIN')) {
            return; // Disallow.
        } elseif (empty($GLOBALS[MEGAOPTIM_RAPID_CACHE_GLOBAL_NS.'_uninstalling'])) {
        ...
        $like = '%'.$wpdb->esc_like(MEGAOPTIM_RAPID_CACHE_GLOBAL_NS).'%';
     ...
     *    existing set of website files (like the AWS Elastic Beanstalk does) will NOT cause Rapid Cache
     *    to stop functioning due to the lack of an `advanced-cache.php` file, which is generated by Rapid Cache.
     *
     ...
     *    Rapid Cache creates this file on its own. Thus, if it's missing (and CC is active)
     *    we simply regenerate the file automatically to keep Rapid Cache running.
     */
    public function checkAdvancedCache()
    {
        if (!$this->options['enable']) {
            return; // Nothing to do.
        } elseif (!empty($_REQUEST[MEGAOPTIM_RAPID_CACHE_GLOBAL_NS])) {
            return; // Skip on plugin actions.
        }
        $cache_dir                 = $this->cacheDir();
        $advanced_cache_file       = WP_CONTENT_DIR.'/advanced-cache.php';
        $advanced_cache_check_file = $cache_dir.'/'.mb_strtolower(MEGAOPTIM_RAPID_CACHE_SHORT_NAME).'-advanced-cache';
     ...
     *                   that is specifically related to the `[MEGAOPTIM_RAPID_CACHE_SHORT_NAME]-advanced-cache` file.
     */
    public function addAdvancedCache()
    {
        ...
        $advanced_cache_file       = WP_CONTENT_DIR.'/advanced-cache.php';
        $advanced_cache_check_file = $cache_dir.'/'.mb_strtolower(MEGAOPTIM_RAPID_CACHE_SHORT_NAME).'-advanced-cache';
        $advanced_cache_template   = MEGAOPTIM_RAPID_CACHE_PATH.'stubs/advanced-cache.x-php';
        ...
        $possible_advanced_cache_constant_key_values = array_merge(
            ...
            [
                ...
                'ac_file_version' => MEGAOPTIM_RAPID_CACHE_VERSION,  // Version when AC file was built.
            ]
        );
        if ($this->applyWpFilters(MEGAOPTIM_RAPID_CACHE_GLOBAL_NS.'_exclude_uris_client_side_too', true)) {
        ...
        foreach ($possible_advanced_cache_constant_key_values as $_option => $_value) {
            ...
            $advanced_cache_contents = preg_replace(
                [
                    '/'.preg_quote("'%%".MEGAOPTIM_RAPID_CACHE_GLOBAL_NS.'_'.$_option."%%'", '/').'/ui',
                    '/'.preg_quote("'%%".MEGAOPTIM_RAPID_CACHE_GLOBAL_NS.'_'.preg_replace('/^cache_/ui', '', $_option)."%%'", '/').'/ui',
                ],
                $_value,
        ...
        if (mb_strpos(MEGAOPTIM_RAPID_CACHE_PLUGIN_FILE, WP_CONTENT_DIR) === 0) {
            $plugin_file = "WP_CONTENT_DIR.'".$this->escSq(str_replace(WP_CONTENT_DIR, '', MEGAOPTIM_RAPID_CACHE_PLUGIN_FILE))."'";
        } else {
            $plugin_file = "'".$this->escSq(MEGAOPTIM_RAPID_CACHE_PLUGIN_FILE)."'"; // Full absolute path.
        }
        // Make it possible for the `advanced-cache.php` handler to find the plugin directory reliably.
        $advanced_cache_contents = preg_replace('/'.preg_quote("'%%".MEGAOPTIM_RAPID_CACHE_GLOBAL_NS."_PLUGIN_FILE%%'", '/').'/ui', $plugin_file, $advanced_cache_contents);

        // Ignore; this is created by Rapid Cache; and we don't need to obey in this case.
    ...
    public function removeAdvancedCache()
    {
        ...
        /* Empty the file only. This way permissions are NOT lost in cases where
            a site owner makes this specific file writable for Rapid Cache. */
    ...
    /**
     * Checks to make sure the `[MEGAOPTIM_RAPID_CACHE_SHORT_NAME]-blog-paths` file still exists;
     *    and if it doesn't, the `[MEGAOPTIM_RAPID_CACHE_SHORT_NAME]-blog-paths` file is regenerated automatically.
     *
     * @since 1.0.0
     *
     * @attaches-to `init` hook.
     ...
     * @note This runs so that remote deployments which completely wipe out an
     *    existing set of website files (like the AWS Elastic Beanstalk does) will NOT cause Rapid Cache
     *    to stop functioning due to the lack of a `[MEGAOPTIM_RAPID_CACHE_SHORT_NAME]-blog-paths` file, which is generated by Rapid Cache.
     *
     *    For instance, if you have a Git repo with all of your site files; when you push those files
     *    to your website to deploy them, you most likely do NOT have the `[MEGAOPTIM_RAPID_CACHE_SHORT_NAME]-blog-paths` file.
     *    Rapid Cache creates this file on its own. Thus, if it's missing (and CC is active)
     *    we simply regenerate the file automatically to keep Rapid Cache running.
     */
    public function checkBlogPaths()
    {
        ...
        $cache_dir       = $this->cacheDir();
        $blog_paths_file = $cache_dir.'/'.mb_strtolower(MEGAOPTIM_RAPID_CACHE_SHORT_NAME).'-blog-paths';
    ...
    /**
     * Creates and/or updates the `[MEGAOPTIM_RAPID_CACHE_SHORT_NAME]-blog-paths` file.
     *
     * @since 1.0.0
     *
