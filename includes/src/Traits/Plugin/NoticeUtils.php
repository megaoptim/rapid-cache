...
trait NoticeUtils
{
    ...
    public function allAdminNotices()
    {
        ...
        foreach ($notices as $_key => $_notice) {
            ...
            if ($_notice['persistent_key'] && $_notice['dismissable']) { // See above. The `dismissNotice()` action requires `$this->cap` always.
                $_dismiss = add_query_arg(urlencode_deep([MEGAOPTIM_RAPID_CACHE_GLOBAL_NS => ['dismissNotice' => ['key' => $_key]], '_wpnonce' => wp_create_nonce()]));
                $_dismiss = '<a href="'.esc_attr($_dismiss).'"><button type="button" class="notice-dismiss"><span class="screen-reader-text">'.__('Dismiss this notice.', 'rapid-cache').'</span></button></a>';
            }
        ...
        if (!empty($combined_notices)) { // Display a single notice with details hidden by default.
            ...
            $_see_details  = __('See details.', 'rapid-cache');
            $_hide_details = __('Hide details.', 'rapid-cache');
    ...
    public function getNotices($blog_id = 0)
    {
        if (is_multisite()) {
            ...
            $notices     = get_site_option(MEGAOPTIM_RAPID_CACHE_GLOBAL_NS.$blog_suffix.'_notices');
        } else {
            $notices = get_site_option(MEGAOPTIM_RAPID_CACHE_GLOBAL_NS.'_notices');
        }
        if (!is_array($notices)) {
            $notices = []; // Force array.
    ...
    public function updateNotices(array $notices, $blog_id = 0)
    {
        if (is_multisite()) {
            ...
            $blog_suffix = '_'.$blog_id; // Site option suffix.
            update_site_option(MEGAOPTIM_RAPID_CACHE_GLOBAL_NS.$blog_suffix.'_notices', $notices);
        } else {
            update_site_option(MEGAOPTIM_RAPID_CACHE_GLOBAL_NS.'_notices', $notices);
        }
        return $notices;
    }
