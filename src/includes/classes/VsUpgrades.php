...
class VsUpgrades extends AbsBase
{
    ...
    protected function fromLte151114()
    {
        if (version_compare($this->prev_version, '151114', '<=')) {
            ...
            if (!$this->plugin->findHtaccessMarker('Rapid Cache')) {
                return; // Template blocks are already gone.
            }
            if ($htaccess = $this->plugin->readHtaccessFile($htaccess_file)) {
                if (is_dir($templates_dir = MEGAOPTIM_RAPID_CACHE_PATH.'stubs/htaccess/back-compat')) {
    ...
    /**
     * Before we changed the name to Rapid Cache.
     *
    ...
    protected function fromZenCache()
    {
        if (is_array($zencache_options = get_site_option('zencache_options'))) {
            ...
            // Remove htaccess rules added by ZenCache so that they can be re-added by Rapid Cache
            ...
            $this->plugin->enqueueMainNotice(
                '<p>'.sprintf(__('<strong>Woohoo! %1$s activated.</strong> :-)', 'rapid-cache'), esc_html(NAME)).'</p>'.
                '<p>'.sprintf(__('NOTE: Your ZenCache options were preserved by %1$s (for more details, visit the <a href="%2$s" target="_blank">Migration FAQ</a>).'.'', 'rapid-cache'), esc_html(NAME), esc_attr(IS_PRO ? 'http://rapidcache.com/r/zencache-pro-migration-faq/' : 'https://rapidcache.com/r/zencache-migration-faq/')).'</p>'.
                '<p>'.sprintf(__('To review your configuration, please see: <a href="%2$s">%1$s → Plugin Options</a>.'.'', 'rapid-cache'), esc_html(NAME), esc_attr(add_query_arg(urlencode_deep(['page' => GLOBAL_NS]), self_admin_url('/admin.php')))).'</p>'
            );
        }
    }
     ...
     * and before we renamed RAPID_CACHE_ALLOW_BROWSER_CACHE to RAPID_CACHE_ALLOW_CLIENT_SIDE_CACHE.
     *
     * @since 160706
     */
