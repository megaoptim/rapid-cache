...
trait InstallUtils
{
    ...
    public function activate()
    {
        ...
        if (!$this->options['welcomed'] && !$this->options['enable']) {
            ...
            $this->enqueueMainNotice(sprintf(__('<strong>%1$s</strong> successfully installed! :-) <strong>Please <a href="%2$s">enable caching and review options</a>.</strong>', 'rapid-cache'), esc_html(NAME), esc_attr($settings_url)), ['push_to_top' => true]);
    ...
    public function checkVersion()
    {
        ...
        if (is_admin()) { // Only if in an admin area.
            $this->enqueueMainNotice(sprintf(__('<strong>%1$s:</strong> detected a new version of itself. Recompiling w/ latest version... wiping the cache... all done :-)', 'rapid-cache'), esc_html(NAME)), ['push_to_top' => true]);
        }
     ...
     * @note This runs so that remote deployments which completely wipe out an
     *    existing set of website files (like the AWS Elastic Beanstalk does) will NOT cause Rapid Cache
     *    to stop functioning due to the lack of a `[SHORT_NAME]-blog-paths` file, which is generated by Rapid Cache.
     *
     *    For instance, if you have a Git repo with all of your site files; when you push those files
